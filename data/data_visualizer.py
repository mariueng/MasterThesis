...
def get_columns_data(data, sub_markets, resolution):
    ...
    for data_source in data:
        if data_source == "Price":
            ...
            for m in sub_markets:
                if m in hydro_markets:
                    included_cols.append(m + " Hydro Dev")
        else:
            for m in sub_markets:
...
def append_markets_title(data, markets):
    if len(data) == 2:
        return ""
    replace_dict_countries = {"NO": "Norway", "SE": "Sweden", "FI": "Finland", "DK": "Denmark", "EE": "Estonia",
                              "LV": "Latvia", "LT": "Lithuania"}
    if len(markets) == 1:  # plotting for only one country/market
        title = markets[0]
        for key, value in replace_dict_countries.items():
